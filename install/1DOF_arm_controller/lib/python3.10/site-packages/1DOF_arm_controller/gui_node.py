...
import threading
from .data_plot import DrawingApp
...
class MyGUI:
    def __init__(self, root, node):
        self.root = root
        self.node = node
        self.drawing = DrawingApp(root)
        self.dropdown_var = tk.StringVar(root)
        self.create_widgets()
        self.Stop_motor()
        self.data = self.node.sensor_data.current

    def create_widgets(self):
        # Dropdown Menu
        options = ["1:Assistive Mode", "2:Passive Mode", "3:Resistive ModeA", "4:Resistive ModeB"]
        ...
        label1.grid(row=0, column=0, padx=10, pady=5, sticky='ew')

        self.spinbox1 = tk.Spinbox(self.root, from_=0, to=100)
        self.spinbox1.grid(row=0, column=1, padx=10, pady=5, sticky='ew')

        # Entry 2 : For Start point
        label2 = tk.Label(self.root, text="Start point:")
        label2.grid(row=1, column=0, padx=10, pady=5, sticky='ew')
        ...
        self.read_start_point.grid(row=1, column=1, padx=10, pady=5, sticky='ew')

        submit_button = tk.Button(self.root, text="update", command=self.update_start_position)
        submit_button.grid(row=1,column=2, sticky='ew')

        # Entry 3 : For End Point
        label3 = tk.Label(self.root, text="End point:")
        label3.grid(row=2, column=0, padx=10, pady=5, sticky='ew')

        self.entry_end_point = tk.IntVar()
        # entry_variable.set(self.update_position())

        self.read_end_point = tk.Entry(self.root,textvariable=self.entry_end_point)
        self.read_end_point.grid(row=2, column=1, padx=10, pady=5, sticky='ew')

        submit_button = tk.Button(self.root, text="update", command=self.update_end_position)
        submit_button.grid(row=2,column=2, sticky='ew')

        # Entry 4 : For Motion Speed
        label4 = tk.Label(self.root, text="Speed:")
        label4.grid(row=3, column=0, padx=10, pady=5, sticky='ew')

        self.spinbox4 = tk.Spinbox(self.root, from_=0, to=3000)
        self.spinbox4.grid(row=3, column=1, padx=10, pady=5, sticky='ew')

        # Entry 5 : For Effore/Torque
        label4 = tk.Label(self.root, text="Torque:")
        label4.grid(row=4, column=0, padx=10, pady=5, sticky='ew')

        self.spinbox5 = tk.Spinbox(self.root, from_=0, to=3000)
        self.spinbox5.grid(row=4, column=1, padx=10, pady=5, sticky='ew')

        # Entry 6 : For threshold 
        label6 = tk.Label(self.root, text="Torque Threshold:")
        label6.grid(row=5, column=0, padx=10, pady=5, sticky='ew')

        self.spinbox6 = tk.Spinbox(self.root, from_=0, to=3000)
        self.spinbox6.grid(row=5, column=1, padx=10, pady=5, sticky='ew')

        # Entry 7 : For threshold 
        label7 = tk.Label(self.root, text="Delay:")
        label7.grid(row=6, column=0, padx=10, pady=5, sticky='ew')

        self.spinbox7 = tk.Spinbox(self.root, from_=0, to=3000)
        self.spinbox7.grid(row=6, column=1, padx=10, pady=5, sticky='ew')

        # =======================Button Widgets======================= 
        # Run Motor
        self.publish_button = ttk.Button(self.root, text="Run Motor", command=self.submit_action)
        self.publish_button.grid(row=7,column=1)
        ...
        self.sensor_label.grid(row=7,column=3, sticky='ew')

        # ======================Sensor Data Animation================
        # Create Canvas and sensor data plotting
    ...
    def update_anime(self):
        if self.node.sensor_data:
            ...
            self.drawing.create_rect_anime(self.node.sensor_data.current,9,1)
        self.root.after(100, self.update_anime)
    ...
    def submit_action(self):
        ...
        app_param.torque = int(self.spinbox5.get())
        app_param.threshold = int(self.spinbox6.get())
        app_param.delay = int(self.spinbox7.get())
    ...
    def update_sensor_data(self):
        if self.node.sensor_data:
            self.sensor_label.config(text=f"Sensor Data: \nCurrent: {round(self.node.sensor_data.current,2)}\nPosition: {round(self.node.sensor_data.position)}\nVelocity: {round(self.node.sensor_data.velocity)}")
            # self.drawing.create_rect_anime(self.node.sensor_data.current,6,1)
        self.root.after(300, self.update_sensor_data)
